...
const scan = async (packageJsonPath, reportPath) => {
  ...
  if (reportPath) {
    writeFileSync(`${path}/js-green-licenses.json`, JSON.stringify({ greenLicenses: [] }))
    ...
    try {
      ...
      writeFileSync(
        reportPath,
        licenses.reduce(
          (result, { packageName, version, licenseName }) => `${result}\n${packageName}@${version} ${licenseName}`,
          ''
        )
      )
  ...
  copyFileSync(`./tools/js-green-licenses.json`, `${path}/js-green-licenses.json`)
  try {
    const licenses = await runCheck(path, true)
    if (licenses.length > 0) {
      licenses.map(({ packageName, version, licenseName }) =>
        console.log(`Found invalid license for ${packageName}@${version}: ${licenseName}`)
      )
...
;(async () => {
  if (process.argv[2] === 'report') {
    await scan('./frontend/package.json', process.argv[2] === 'report' && './client/build/licenses.txt')
  } else {
    await scan('./server/package.json')
    await scan('./client/package.json')
  }
})()
