...
export class InfraPipelineStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: InfraPipelineProperties) {
    ...
    const githubConnectionArn = StringParameter.valueForStringParameter(this, '/agency-demo/github-connection-arn')
    const source = CodePipelineSource.connection('findy-network/agency-demo', 'master', {
      ...
      synth: new CodeBuildStep('SynthStep', {
        ...
        rolePolicyStatements: [
          new PolicyStatement({
            ...
            resources: [`arn:aws:ssm:${this.region}:${this.account}:parameter/agency-demo*`],
          }),
        ],
  ...
  createFrontendBuildStep() {
    return new CodeBuildStep('FindyAgencyDemoBuildFrontendStep', {
      ...
      commands: ['apk add bash', 'yarn install', 'yarn client:build'],
      buildEnvironment: {
        buildImage: codebuild.LinuxBuildImage.fromDockerRegistry('node:16.18-alpine3.16'),
        environmentVariables: {
